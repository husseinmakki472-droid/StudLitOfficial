...
export default function Pricing({ dark = true }) {
  ...
  const cardBorder       = dark ? '1px solid rgba(255,255,255,0.1)' : '1px solid #d4c9fb'
  const cardShadow       = dark ? '0 0 60px rgba(124,92,252,0.18)' : '0 8px 48px rgba(124,92,252,0.18), 0 2px 8px rgba(0,0,0,0.04)'
  const dividerColor     = dark ? 'rgba(255,255,255,0.09)'     : '#d4c9fb'
  const priceSymColor    = dark ? 'rgba(240,239,255,0.5)'      : '#6b68a0'
  const pricePeriodColor = dark ? 'rgba(240,239,255,0.35)'     : '#6b68a0'
  const featureColor     = dark ? 'rgba(240,239,255,0.65)'     : '#3b3763'
  const cancelColor      = dark ? 'rgba(240,239,255,0.3)'      : '#6b68a0'
  const badgeColor       = dark ? '#b06ef3'                    : '#6d3fc9'
  const trialBadgeBg     = dark ? 'rgba(74,222,128,0.1)'       : 'rgba(22,163,74,0.1)'
  const trialBadgeBorder = dark ? '1px solid rgba(74,222,128,0.25)' : '1px solid rgba(22,163,74,0.3)'
  const trialBadgeColor  = dark ? '#4ade80'                    : '#16a34a'
  const freeLineColor    = dark ? '#4ade80'                    : '#16a34a'
  const checkBg          = dark ? 'rgba(124,92,252,0.15)'      : 'rgba(124,92,252,0.12)'
    ...
    <section id="pricing" style={{ position: 'relative', zIndex: 1, maxWidth: 520, margin: '0 auto', padding: '0 28px 96px', textAlign: 'center' }}>
      ...
      <div ref={cardRef} style={{ ...cardReveal }}>
        <div style={{
          ...
          <div style={{
            display: 'inline-flex', alignItems: 'center', gap: 6,
            background: trialBadgeBg, border: trialBadgeBorder,
            borderRadius: 999, padding: '5px 14px',
            fontFamily: 'Inter, sans-serif', fontSize: 12, fontWeight: 700,
            color: trialBadgeColor, marginBottom: 20,
          }}>
          ...
          <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, color: freeLineColor, fontWeight: 600, marginBottom: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
            ...
            {FEATURES.map(f => (
              <li key={f} style={{ display: 'flex', alignItems: 'center', gap: 11, fontFamily: 'Inter, sans-serif', fontSize: 14, color: featureColor }}>
                <span style={{
                  ...
                  background: checkBg, color: '#7c5cfc',
