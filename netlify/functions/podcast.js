export const handler = async (event) => {
  ...
  try {
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o', max_tokens: 1000, temperature: 0.75,
        messages: [
    ...
    return { statusCode: 200, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ script: data.choices?.[0]?.message?.content || '' }) };
  } catch (err) {
